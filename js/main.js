...
window.onload = function() 
{
    ...
    if (isAndroid)
    {
        v_width = screen.height;
        v_height = screen.width;
    }
    else
    {
       v_width = screen.width/2;
    v_height = screen.height/2; 
    }
        ...
        initialize: function() 
            {
                ...
                Info = new Button("Info","blue");
                Info._style.height = "2em";
                Info._style.width = "4em";
                Info.x = game.width/2.7;
                Info.y = 5;

                Info.addEventListener("touchstart", this.touchToInfo);

                Exit = new Button("Exit","blue");
                Exit._style.height = "2em";
                Exit._style.width = "4em";

                Exit.x = game.width/2.7;
                Exit.y = v_height - 50;
                
                Exit.addEventListener('exitframe', this.touchToExit);
    ...
    var SceneInfoGame = Class.create(Scene, 
    {
        initialize: function() 
        {
                ...
                var Aboutlabel = new Label("Игра рассчитана на игру ТОЛЬКО<br>одним человеком<br>иначе теряет весь смысл.<br>Цель игры -<br>продержаться как можно дольше<br>избегая препятствия.");
                Aboutlabel.x = 0;
                Aboutlabel.y = 50;
                Aboutlabel.textAlign = 'left';
                Aboutlabel.font = 'italic bold 16px sans-serif';

                var Controlslabel = new Label("Управление на ПК осуществляется<br> клавишами 'A' и 'D' для левого<br> пингвинчика и стрелочками<br>влево-вправо - для правого пингвинчика. <br> В версии для Android-девайсов<br>управление осущевствляется зажатием<br>пингвина пальцем и смещение<br>его в сторону.");
                Controlslabel.x = v_width - game.width/2;
                Controlslabel.y = 50;
                Controlslabel.textAlign = 'right';
                Controlslabel.font = 'italic bold 14px sans-serif'

                var Autorlabel = new Label("Приятной игры! <br> Игра разработана с<br>помощью enchant.js,<br> JS, HTML, CSS <br> 2015.");
                Autorlabel.x = v_width/4;
                Autorlabel.y = v_height - 100;
                Autorlabel.font = 'italic bold 14px sans-serif'
                Autorlabel.textAlign = 'center';

                back = new Button("back","blue");
